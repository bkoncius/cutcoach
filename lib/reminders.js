 ...
 *   weightToday, trendWeight, weeklyRate,
...
function weighInCopy(ctx) {
  ...
  const verdict = onTrack(ctx.phase, ctx.weeklyRate, ctx.trendWeight)
    ? "Right in the lane."
    : "Off the lane.";
  return {
    title: "Weigh in",
    body: `Trend ${fmt1(ctx.trendWeight)} kg · ${rate}. ${verdict} Scale, then coffee.`,
    tab: "today",
  };
}
