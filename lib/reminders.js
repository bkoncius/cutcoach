...
// Is the 7-day trend moving the way this phase wants? Exported so the Today tab and
// the notification copy can't disagree about what "on track" means.
export function onTrack(phase, rate) {
  if (rate == null) return true;
  if (phase === "bulk") return rate > 0;
  if (phase === "maintain") return Math.abs(rate) <= 0.25;
  return rate < 0; // cut
}
...
function weighInCopy(ctx) {
  ...
  if (ctx.avg7 == null || ctx.weeklyRate == null) {
    return {
      title: "Weigh in",
  ...
  const rate = `${sign}${fmt1(Math.abs(ctx.weeklyRate))} kg/wk`;
  const verdict = onTrack(ctx.phase, ctx.weeklyRate) ? "Right in the lane." : "Off the lane.";
  return {
    title: "Weigh in",
    body: `7-day avg ${fmt1(ctx.avg7)} kg · ${rate}. ${verdict} Scale, then coffee.`,
    tab: "today",
  };
}
...
function trainCopy(ctx, cfg) {
  ...
  const next = PROGRAM[ctx.nextTemplate];
  const nextName = next ? next.name : "Your next session";
...
function checkinCopy(ctx) {
  ...
  const trained =
    ctx.trainedToday && ctx.todayTemplate && PROGRAM[ctx.todayTemplate]
      ? ` · trained ${PROGRAM[ctx.todayTemplate].name}`
      : "";
  return {
    title: "Check-in time",
