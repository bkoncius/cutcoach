...
export function laneText(phase, weightKg, profile = {}) {
  const lane = laneFor(phase, weightKg, profile);
  const p = phase || "cut";
  if (!lane) {
    return p === "cut"
  ...
  if (p === "cut") {
    return `Healthy lane: ${fmt2(lane.minKg)} to ${fmt2(lane.maxKg)} kg/week on the trend. Slower for 2–3 weeks → tighten intake; faster than ${fmt2(lane.minKg)} → eat a bit more, the muscle is the point.`;
  }
  if (p === "bulk") {
    return `Lean-gain lane: ${fmt2(lane.minKg)} to ${fmt2(lane.maxKg)} kg/week on the trend. Flat for 2–3 weeks → add a little; faster than ${fmt2(lane.maxKg)} → trim, the excess is mostly fat.`;
  }
  return `Lane: hold within ${fmt2(lane.minKg)} to ${fmt2(lane.maxKg)} kg/week on the trend while pushing your lifts.`;
}
